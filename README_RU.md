...
> [Онлайн симулятор](https://wokwi.com/projects/447704861114129409). Заметьте, что в эмуляторе устройством удобнее управлять через клавиатуру
...
1. Лаг запуска лампы. Нажми кнопку `Start` в настройках, чтобы проверить его
2. Яркость подсветки
3. Громкость пищалки
4. Время автоотключения лампы в режиме просмотра
5. Возможность стартовать с настроек
6. Выбор анимации на режим простоя.  Нажми кнопку `Start` в настройках, чтобы включить предпросмотр
...
9. Экспертные тестовые режимы (отключены по-умолчанию): включают функционал всех тестовых режимов. Позволяют задать кастомное время первой засветки (префлеш, часть сплит-грейд тестов); указать какого типа будут тесты (локализованные или обычные). Можно указать их вместо всех стадартных тестых режимов. Единственный недостаток этого режима -- большое количество под-меню
...
9. Красный LCD 1602.
    * [LCD без I2C адаптера](https://sl.aliexpress.ru/p?key=tgDHsiN). Сложнее при пайке, но имеет лучший угол обзора в сравнении со следующим. Использует дополнительный резистор для регулировки контраста: 2.2 кОм (значение резистра может отличаться от моего). Можете припаять свой I2C адаптер для удобства пайки
...
### Пищалка постоянно пищит. Даже когда ничего не происходит

Определите это в UserConfig.h:
```cpp
#define BEEPER_VOLUME_SILENT 0
```
...
`UserConfig.h` должен быть создан рядом с `DarkroomTimer.ino`. Информацию о том, что можно сконфигурировать есть в `UserConfig.example.h`
